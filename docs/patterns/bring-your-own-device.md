...
- Protected Utility blueprint AVD pattern to be deployed for the virtual desktop pattern, see [Azure Virtual Desktop pattern](./azure-virtual-desktop.md)
...
Note, refer to the [System Security Plan](../security/system-security-plan.md) for full traceability against all controls for the Protected Utility blueprint outside of this pattern.
...
The recommendation to implement this option is to utilise the [Azure Virtual Desktop blueprint](azure-virtual-desktop.md) pattern. The solution requires architectural decisions to be made that are outside the scope of this pattern. This BYOD option is more difficult to implement than the MAM BYOD option 1, but offers a less complex implementation than other virtual desktop solutions.
...
Further implementation advice is provided in the Protected Utility blueprint [Azure Virtual Desktop pattern](azure-virtual-desktop.md). This guide is a bolt-on pattern to the Protected Utility blueprint.